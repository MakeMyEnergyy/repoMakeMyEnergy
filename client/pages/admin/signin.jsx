...
export default function SignIn() {
    ...
    const postSigninData = async (e) => {
        ...
        await axios.post('/route/adminSignin', submitSigninData).then((response) => {
            if (response.status === 200 && response.data === "Authentication Successful") {
                localStorage.setItem("makemyenergy_Admin_Email", signinData.email)
                router.push('/admin/users')
            }
        }).catch(error => {
                    ...
                    <div className='w-[100vw] bg-white'>
                        <SimpleGrid columns={[null, 1, 2]} spacing={4} maxW='6xl' className='mx-auto mt-10' >
                            <GridItem className='mt-10 px-10'>
                                <p className='text-black font-bold text-3xl'>Admin Sign in</p>
                                    ...
                                    <div class="w-full max-w-full">
                                        <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" onSubmit={e => postSigninData(e)}>
                                            ...
                                            <div class="flex items-center justify-between">
                                                ...
                                                <Link href="#" class="inline-block align-baseline font-bold text-base text-blue-500 hover:text-blue-800" >
                                                    Forgot Password?
                                                </Link>
                                            </div>
                                        </form>
