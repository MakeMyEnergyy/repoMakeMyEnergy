...
import img1 from "../../public/meterboard/do.png"
import img2 from "../../public/meterboard/donot.png"
...
export default function MeterBoard() {
    ...
    const [meterBoardUrl, setMeterBoardUrl] = useState(null)
    ...
    useEffect(() => {
        if (localStorage.getItem("makemyenergy_Email") === null) {
            ...
            axios.get(`/route/getUser/${localStorage.getItem("makemyenergy_Email")}`, { withCredentials: true })
                .then((res) => setMeterBoardUrl(res.data[0].meterboard_url))
                .catch(async (error) => {
            ...
            axios.get(`/route/getUser/${localStorage.getItem("makemyenergy_Email")}`, { withCredentials: true })
                .then((res) => setWallLocation(res.data[0].meter_standing_pos))
                .catch(async (error) => {
                    if (error.response.status === 401 && (error.response.data === "Access denied, No Token provided" || error.response.data === "Invalid token")) {
                        await axios.get(`/route/logout`).then((response) => {
                            if (response.data === "Logout Successful" && response.status === 200) {
    ...
    const postWallLocation = async (location) => {
        if (location === null || localStorage.getItem("makemyenergy_Email") === null) {
            ...
            await axios.post(`/route/wallPosition/meterboard/${location}/${localStorage.getItem("makemyenergy_Email")}`, { withCredentials: true }).then((response) => {
                if (response.data === "Wall Location Updated" && response.status === 201) {
                    router.push("/customerPanel/inverter")
                }
            }).catch(async (error) => {
                if (error.response.status === 401 && (error.response.data === "Access denied, No Token provided" || error.response.data === "Invalid token")) {
                    await axios.get(`/route/logout`).then((response) => {
                        ...
                        <Container maxW='5xl' className='mt-12' >
                            ...
                            >
                                ...
                                <GridItem colSpan={3} className="ml-8">
                                    ...
                                    <Box><p className='text-2xl font-bold'>Upload photos of your meterboard</p></Box>
                                    <Box><p className='mt-4 mr-8 text-gray-600'>Give us a few clear photos of your meterboard. Some close up so we can read the numbers, and some further away so we can see the location.</p></Box>
                                    <Box className='mt-4'>
                                        <Grid
                                            templateColumns={{
                                                sm: 'repeat(2,1fr)',
                                                ...
                                                <HStack className='flex'>
                                                    ...
                                                    <Box className='self-start'>
                                                        ...
                                                        <HStack className='mt-2'>
                                                            ...
                                                            <Box className='text-xs text-gray-500'>Give us a clear picture of your meterboard</Box>
                                                        </HStack>
                                                        <HStack className='mt-2'>
                                                            <Box>
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-[#21376C]">
                                                            ...
                                                            <Box className='text-xs text-gray-500'>Take a picture that shows the location of your meterboard</Box>
                                                        </HStack>
                                                    </Box>
                                                </HStack>
                                            </GridItem>
                                                ...
                                                <HStack className='flex'>
                                                    ...
                                                    <Box className='self-start'>
                                                        ...
                                                        <HStack className='mt-2'>
                                                            ...
                                                            <Box className='text-xs text-gray-500'>Just send an extreme close-up</Box>
                                                        </HStack>
                                                    </Box>
                                                </HStack>
                                            </GridItem>
                                    ...
                                    <Box className='m-1'>
                                        ...
                                        <div className='flex w-[40vw] flex-row'>
                                            <Box className=' w-[300px] -ml-4' >
                                                <UploadImage route={"meterboard_url"} />
                                            </Box>
                                            <div className='w-[40vw] m-4'>
                                                {(meterBoardUrl !== null) ? <img src={meterBoardUrl} alt="uploaded image" className='h-[25vh]' /> : <p></p>}
                                            </div>
                                        </div>
                                    </Box>
