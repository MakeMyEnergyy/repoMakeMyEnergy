...
export default function EditPanel({ singlePanel }) {
    ...
    const {
      ...
      defaultValues: {
        fields: singlePanel.size.map((value, index) => ({
          size: value,
          price: singlePanel.price[index],
          no_of_panels: singlePanel.no_of_panels[index],
        })),
      },
    });
    ...
    async function onSubmit(data) {
        ...
        for (var i = 0; i < data.fields.length; i++) {
          ...
          panel_number[i] = data.fields[i].no_of_panels;
        }
        ...
        data.size=size
        data.price=price
        data.no_of_panels= panel_number
        data.id=singlePanel._id
        data.brand=singlePanel.brand
        
        console.log(data)
  
        try {
          const response = await axios.post(`/route/updateProduct/panel`, data,{
                  ...
                  <SimpleGrid columns={2} spacingX="40px" spacingY="20px">
                    <Box>
                      <FormControl>
                        ...
                        <Input
                          ref={initialRef}
                          defaultValue={singlePanel.model}
                    ...
                    <Box>
                      <FormControl>
                        ...
                        <Input
                          ref={initialRef}
                          defaultValue={singlePanel.product_desc}
                    ...
                    <Box>
                      <FormControl>
                        ...
                        <Input
                          ref={initialRef}
                          defaultValue={singlePanel.prod_warranty}
                    ...
                    <Box>
                      <FormControl>
                        ...
                        <Input
                          ref={initialRef}
                          defaultValue={singlePanel.efficiancy_warranty}
                    ...
                    {fields.map((item, index) => (
                      <div key={index} className="border-2 border-gray-400 p-2 m-2 rounded-xl">
                        <Box>
                          <FormControl>
                            <FormLabel>
                              Panel {index + 1}- Number Of Panel 
                            </FormLabel>
                            <Input
                              ref={initialRef}
                              {...register(`fields.${index}.no_of_panels`)}
                              defaultValue={item.no_of_panels}
                            />
                          </FormControl>
                        </Box>
                    ...
                    <Button
                      ...
                      variant="outline"
                      type="button"
                      onClick={() => append({ size: '', price: '', no_of_panels: '' })}
                    >
