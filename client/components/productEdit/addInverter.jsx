...
export default function AddInverter() {
    ...
    async function onSubmit(data) {
      ...
      let price = [data.first_price];
        ...
        form.append("pdf_url", data.pdf[0]);
        ...
        try {
          const response = await axios.post(`http://localhost:3050/route/addProduct/inverter`, form);
    ...
    return (
      <>
        <Button colorScheme="orange" onClick={onOpen}>
          <BsPlusLg className="font-bold mr-3" /> Add Inverter
        </Button>
        <Modal isOpen={isOpen} onClose={onClose} size="xl">
          <ModalOverlay />
          <form onSubmit={handleSubmit(onSubmit)}>
            <ModalContent>
              <ModalHeader>Add Inverter</ModalHeader>
              <ModalCloseButton />
              <ModalBody pb={6}>
                <Box>
                  <FormControl>
                ...
                >
                  ...
                  <div className="border-2 border-gray-400 p-2 m-2 rounded-xl">
                    <Box>
                      <FormControl>
                        <FormLabel>Inverter 1 - Size</FormLabel>
                        <Input
                          ref={initialRef}
                          placeholder="Inverter Size"
                          {...register(`first_size`, {
                            required: true,
                          })}
                        />
                    ...
                    <Box>
                      <FormControl>
                        <FormLabel>Inverter 1 - Price</FormLabel>
                        <Input
                          ref={initialRef}
                  ...
                  {array1Fields.map((item, index) => (
                    ...
                    >
                      <Box>
                        <FormControl>
                          <FormLabel>Inverter {index + 2} - Size</FormLabel>
                          <Input
                            ref={initialRef}
                            placeholder="Inverter Size"
                      ...
                      <Box>
                        <FormControl>
                          <FormLabel>Inverter {index + 2}- Price</FormLabel>
                          <Input
                            ref={initialRef}
                            placeholder="Inverter Price"
