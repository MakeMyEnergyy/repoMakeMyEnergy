...
export default function AddBattery() {
    ...
    async function onSubmit(data) {
        ...
        data.with_esp=Number(data.with_esp)
        data.without_esp=Number(data.without_esp)
        ...
        try {
          const response = await axios.post(`http://localhost:3050/route/addProduct/battery`, data);
    ...
    return (
      <>
        <Button colorScheme="orange" onClick={onOpen}>
          <BsPlusLg className="font-bold mr-3" /> Add Battery
        </Button>
        <Modal isOpen={isOpen} onClose={onClose} size="xl">
          <ModalOverlay />
          <form onSubmit={handleSubmit(onSubmit)}>
            <ModalContent>
              <ModalHeader>Add Battery</ModalHeader>
              <ModalCloseButton />
              <ModalBody pb={6}>
                <Box>
                  <FormControl>
                ...
                >
                  ...
                  <Box>
                    <FormControl>
                      <FormLabel>Without ESP Price</FormLabel>
                      <Input
                        
                        placeholder="Without ESP"
                        {...register("without_esp", { required: true })}
                      />
                    </FormControl>
                  </Box>
                  <Box>
                    <FormControl>
                      <FormLabel>With ESP Price</FormLabel>
                      <Input
                        
                        placeholder="With ESP"
                        {...register("with_esp", { required: true })}
                      />
                    </FormControl>
                  </Box>
                  <Box>
